...
/**
 * 初始化 Protobuf 解析器
 * 从 resources 目录加载 proto 文件
 */
export async function initProtoDecoder(): Promise<void> {
  try {
    // 使用内联的 proto 定义，避免文件路径问题
    const protoContent = `
syntax = "proto3";
...
`;
    ...
    console.log('Protobuf decoder initialized with inline definitions');
  } catch (error) {
...
/**
 * 解码 Protobuf 消息
 * @param topic MQTT 主题（就是消息类型名）
 * @param buffer 消息二进制数据
 * @returns 解码后的 JSON 对象
 */
export function decodeMessage(topic: string, buffer: Uint8Array | Buffer): any {
  if (!root) {
  ...
  try {
    ...
    console.error(`[Proto] Failed to decode message ${topic}:`, error);
    return null;
  }
}
 ...
 * 获取消息类型名称
 */
export function getMessageTypeName(topic: string): string {
  // 主题名就是消息类型
  return topic;
}
